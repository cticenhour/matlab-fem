...
vectorized_method = 0;
geometry = 1; % 1 = box with circle, 2 = box plus square, 3 = box plus square and circle
...
if geometry == 1
    initial_edge_width = wid/30;
    geo_dist_func = @(p) ddiff(drectangle(p,0,wid,0,len),dcircle(p,0.5,0.5,0.2));
    mesh_refinement_func = @(p) 0.04 + 0.3*dcircle(p,0.5,0.5,0.2);

    bounds = [0,0;len,wid];
    important_pts = [0,0;wid,0;0,len;wid,len];
    
    [node_list, triangle_list] = distmesh2d(geo_dist_func,mesh_refinement_func,...
        initial_edge_width,bounds,important_pts);
elseif geometry == 2
    initial_edge_width = wid/20;
    geo_dist_func = @(p) dunion(drectangle(p,0,wid,0,len), drectangle(p,wid,wid+0.5,0,0.5*len));
    bounds = [0,0;len+0.25,wid];
    important_pts = [0,0;wid+0.5,0;0,len;wid+0.5,0.5*len;wid,0.5*len;wid,len];
    
    [node_list,triangle_list] = distmesh2d(geo_dist_func,@huniform,initial_edge_width,bounds,important_pts);
    
elseif geometry == 3
    initial_edge_width = wid/30;
    geo_dist_func = @(p) ddiff(dunion(drectangle(p,0,wid,0,len), drectangle(p,0,wid+0.5,0,0.5*len)),dcircle(p,0.5,0.5,0.2));
    mesh_refinement_func = @(p) 0.04 + 0.3*dcircle(p,0.5,0.5,0.2);
    
    bounds = [0,0;len+0.25,wid];
    important_pts = [0,0;wid+0.5,0;0,len;wid+0.5,0.5*len;wid,0.5*len;wid,len];
    
    [node_list,triangle_list] = distmesh2d(geo_dist_func,mesh_refinement_func,initial_edge_width,bounds,important_pts);
end
