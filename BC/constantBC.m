...
function [K,F] = constantBC(K,F,triangle_list,node_list,port_edges,port_edge_nodes,k,waveguide_width,constant)
            ...
            if is_rF_on_port_boundary == 1 % PORT BOUNDARY CONDITION 
                ...
                if sum(node2) ~= 0 % If node_rF is on a triangle with a full edge on the boundary
                    ...
                    if node_list(node_rF,2) > node_list(node2,2)
                        ...
                        inc_a = constant*exp(1i*sqrt(k^2 - (pi*m/waveguide_width)^2)*node_list(node2,1));
                        inc_half = constant*exp(1i*sqrt(k^2 - (pi*m/waveguide_width)^2)*node_list(node2,1));
                        inc_b = constant*exp(1i*sqrt(k^2 - (pi*m/waveguide_width)^2)*node_list(node_rF,1));
                        ...
                        inc_a = constant*exp(-1i*sqrt(k^2 - (pi*m/waveguide_width)^2)*node_list(node_rF,1));
                        inc_half = constant*exp(-1i*sqrt(k^2 - (pi*m/waveguide_width)^2)*node_list(node_rF,1));
                        inc_b = constant*exp(-1i*sqrt(k^2 - (pi*m/waveguide_width)^2)*node_list(node2,1));
