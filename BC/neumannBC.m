% neumannBC.m
% Add Neumann Boundary Condition to Surfaces
% REQUIRES:
% K = system matrix
...
% value = value of gradient at boundary
% OUTPUTS
% K = updated system matrix
% F = update right hand side vector

function [K,F] = neumannBC(K,F,triangle_list,node_list,exit_edges,exit_edge_nodes,k,waveguide_width,value)
    ...
    for i = 1:size(triangle_list,1)
        ...
        for r = 1:3
            ...
            if is_rF_on_exit_boundary == 1 % ABSORBING BOUNDARY CONDITION            
                ...
                K(node_rF,node_rF) = K(node_rF,node_rF) - (b_minus_a/6)*(trial_a + 4*trial_half + trial_b)*value;
